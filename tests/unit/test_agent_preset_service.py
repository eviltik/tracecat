...
class TestAgentPresetService:
    ...
    async def test_parent_read_warns_when_subagent_has_internet_but_parent_does_not(
        self,
        agent_preset_service: AgentPresetService,
        agent_preset_create_params: AgentPresetCreate,
    ) -> None:
        """Internet-enabled subagents warn when parent internet is disabled."""
        child = await agent_preset_service.create_preset(
            agent_preset_create_params.model_copy(
                update={
                    "name": "Web Child",
        ...
        preset_read = await agent_preset_service.build_preset_read(parent)

        assert len(preset_read.warnings) == 1
        warning = preset_read.warnings[0]
        assert warning.code == "subagent_internet_requires_parent"
        assert warning.subagent_aliases == ["web-research"]
        assert "parent agent does not" in warning.message

    async def test_parent_read_has_no_warning_when_parent_internet_enabled(
        self,
        agent_preset_service: AgentPresetService,
        agent_preset_create_params: AgentPresetCreate,
    ) -> None:
        """Parent internet access satisfies internet-enabled subagents."""
        child = await agent_preset_service.create_preset(
            agent_preset_create_params.model_copy(
                update={
                    "name": "Web Child",
