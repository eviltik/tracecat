---
title: "Workflows"
description: "Build, run, and manage Tracecat workflows: connect triggers and actions, version definitions, and orchestrate automations across cases and agents."
---
...
<Frame caption="Events panel" hint="We only show runs that you triggered manually or by a trigger. This allows other users to work on the same workflow without seeing your runs.">
  ![Workflow events panel](/img/automations/workflows/workflow-events-panel.png)
</Frame>

<br />

<Frame caption="Runs view">
  ![Workflow runs view](/img/automations/workflows/workflow-runs-view.png)
</Frame>

<br />

<Frame caption="History view">
  ![Workflow runs history](/img/automations/workflows/workflow-runs-history.png)
</Frame>
...
![Tag workflow](/img/automations/workflows/tag-workflow.png)
...
### Exported YAML fields

The exported YAML is portable across workspaces. In addition to the workflow definition (`definition`) and visual layout (`layout`), it includes:

- **`alias`** — the workflow alias (human-readable identifier used by `core.workflow.execute` and external tooling). `null` if no alias was set.
- **`folder_path`** — materialized path of the folder the workflow lives in (e.g. `/O365/tests/`). `null` if the workflow is at the workspace root. Lets an importer re-place the workflow in the same folder hierarchy.
- **`tags`** — list of tags attached to the workflow, embedded as full `{ref, name, color}` triplets so the export can be re-imported into a workspace that does not yet know these tags.

The `workflow_id` and `workspace_id` fields are also included so the export records the source of truth, but you can safely strip or replace them when moving the YAML between environments.
